...
const NewBookingModal: React.FC<NewBookingModalProps> = ({ 
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto bg-gradient-to-br from-blue-50 to-green-50 sm:max-w-4xl max-sm:max-w-none max-sm:w-full max-sm:h-full max-sm:max-h-none max-sm:rounded-none max-sm:bg-white">
        <DialogHeader className="text-center pb-6 max-sm:pb-4">
          <DialogTitle className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-green-600 bg-clip-text text-transparent max-sm:text-xl">
            Book Your Cricket Ground
          </DialogTitle>
          <DialogDescription className="text-gray-600 mt-2 max-sm:text-sm max-sm:mt-1">
            Select your preferred date, time, and provide booking details
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-8 max-sm:space-y-6">
          ...
          {ground && (
            <div className="bg-white rounded-xl shadow-md p-6 border border-blue-100 max-sm:p-4 max-sm:rounded-lg">
              <div className="flex items-start justify-between max-sm:flex-col max-sm:gap-3">
                <div className="flex-1">
                  <h3 className="text-xl font-bold text-gray-800 mb-2 flex items-center gap-2 max-sm:text-lg">
                    <MapPin className="w-5 h-5 text-blue-600" />
                    {ground.name}
                  </h3>
                  <p className="text-gray-600 mb-3 max-sm:text-sm">{ground.location.address}</p>
                  <div className="flex items-center gap-4 text-sm max-sm:flex-col max-sm:items-start max-sm:gap-2">
                    <span className="flex items-center gap-1 text-green-600 font-semibold max-sm:text-xs">
                    ...
                    <span className="flex items-center gap-1 text-blue-600 max-sm:text-xs">
                      <Users className="w-4 h-4" />
                      Max {ground.features.capacity} players
                    </span>
                  </div>
                </div>
              </div>
          ...
          {/* Date Selection */}
          <div className="bg-white rounded-xl shadow-md p-6 border border-blue-100 max-sm:p-4 max-sm:rounded-lg">
            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2 max-sm:text-base">
              <Calendar className="w-5 h-5 text-blue-600" />
              Select Date
            </h3>

            <div className="grid grid-cols-7 gap-2 max-sm:grid-cols-3 max-sm:gap-3">
              {getQuickDates().map((date) => (
                <button
                  key={date.toISOString()}
                  onClick={() => setSelectedDate(date)}
                  className={`p-3 rounded-lg text-center transition-all duration-200 max-sm:p-4 ${
                ...
                >
                  <div className="font-medium text-sm max-sm:text-base">{getDateLabel(date)}</div>
                  <div className="text-xs opacity-75 max-sm:text-sm">{format(date, 'MMM d')}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border border-blue-100 max-sm:p-4 max-sm:rounded-lg">
            ...
            {isLoadingSlots ? (
              <div className="text-center text-gray-500 py-8 max-sm:py-6">
                <div className="w-8 h-8 border-2 border-blue-600 border-t-transparent rounded-full animate-spin mx-auto mb-2" />
                <span className="max-sm:text-sm">Loading available slots...</span>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex gap-2">
                  <select
                    className="p-3 rounded-lg border border-gray-200 text-gray-700"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <Label htmlFor="teamName" className="text-sm font-medium text-gray-700 max-sm:text-base">
                Team Name (Optional)
              </Label>
              <Input
                id="teamName"
                ...
                className="rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500 max-sm:h-12 max-sm:text-base"
              />
            </div>

            <div className="space-y-2">
              <Label htmlFor="playerCount" className="text-sm font-medium text-gray-700 max-sm:text-base">
                Number of Players *
              </Label>
              <Input
                id="playerCount"
                type="number"
                min="1"
                max={ground?.features?.capacity || 22}
          ...
          <div className="flex gap-4 pt-4 max-sm:flex-col max-sm:gap-3">
            <Button
              variant="outline"
              onClick={onClose}
              className="flex-1 h-12 rounded-xl border-gray-300 hover:bg-gray-50 text-gray-700 max-sm:h-14 max-sm:text-base"
            >
              Cancel
            </Button>
            <Button
              ...
              className="flex-1 h-12 rounded-xl bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-semibold shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 max-sm:h-14 max-sm:text-base"
            >
              {!selectedDate ||
              !selectedStartSlotObj ||
              !selectedEndTime ||
